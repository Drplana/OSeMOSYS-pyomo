...
def transform_to_hourly(results, bracket_mapping, daytype_mapping, season_mapping):
    ...
    for _, row in results.iterrows():
        try:
            ...
            technology = row['TECHNOLOGY'] # Convertir a string para descomponer
            fuel = row['FUEL']
            year = row['YEAR']
            value = row['value']
            ...
            # Verificar si el bracket existe en bracket_mapping
...
def calculate_yearsplit(season_mapping, daytype_mapping, bracket_mapping, year):
    ...
    yearsplit_df['TIMESLICE'] = yearsplit_df['Season']+yearsplit_df['DayType']+yearsplit_df['DaylyTimeBracket']
    return yearsplit_df
...
def calculate_specified_demand_profile(data):
    ...
    grouped_demand['TIMESLICE'] = grouped_demand['Season']+grouped_demand['DayType']+grouped_demand['DaylyTimeBracket']
...
if __name__ == "__main__":
    ...
    # with pd.ExcelWriter('output4.xlsx') as writer:
        ...
        yearsplit[['TIMESLICE', 'YearSplit']].to_excel(writer, sheet_name='YearSplit', index=False)
        specified_demand_profile[['TIMESLICE', 'SpecifiedDemandProfile']].to_excel(writer, sheet_name='SpecifiedDemandProfile', index=False)
        data.to_excel(writer, sheet_name='DemandData', index=False)
        daysplit.to_excel(writer, sheet_name='DaySplit', index=False)
    

#     results = pd.read_csv('/home/david/OSeMOSYS-pyomo/results/01-BaseScenario/ProductionByTechnology.csv')
#     hourly_results_df = transform_to_hourly(results, bracket_mapping, daytype_mapping, season_mapping)
#     print(hourly_results_df.head())
#     # import matplotlib.pyplot as plt
#     filtered_hourly_data = hourly_results_df[
#         (hourly_results_df['YEAR'] == hourly_results_df['YEAR'].min()) &  # Primer año
#         (hourly_results_df['HOUR'] < 24)  # Primeras 24 horas
#     ]
#     import matplotlib.pyplot as plt
#     # Agrupar por hora y tecnología, y sumar los valores
# # Agrupar por hora y tecnología, y sumar los valores
#     hourly_data_by_tech = filtered_hourly_data.groupby(['HOUR', 'TECHNOLOGY'])['VALUE'].sum().unstack(fill_value=0)

#     # Crear el gráfico de barras apiladas
#     hourly_data_by_tech.plot(
#         kind='bar',
#         stacked=True,
#         figsize=(12, 6),
#         color=plt.cm.tab20.colors,  # Usar un mapa de colores para las tecnologías
#         edgecolor='black'
#     )

#     # Personalizar el gráfico
#     plt.title("Producción Total por Tecnología - Primer Año, Primer Día (24 Horas)", fontsize=14)
#     plt.xlabel("Hora", fontsize=12)
#     plt.ylabel("Producción Total", fontsize=12)
#     plt.xticks(range(0, 24), labels=range(0, 24), rotation=0)  # Mostrar las horas en el eje X
