...
data['Season'] = np.where(data['Month'].isin([12, 1, 2, 3, 4, 5]), 'Winter', 'Summer')
...
cfpv = pd.concat([workdays, weekends]) \
    .groupby(['Month','Season','Weekend','Hour_Group']) \
        ['Average'].mean().dropna()
...
normalized_hourly_sum = hourly_sum_by_season_month_hour_group.div(total_sum)
...
monthly_counts = df.groupby(['Month', 'Type']).size().reset_index(name='Value')
...
monthly_counts = monthly_counts.sort_values(['Month', 'Type'])
